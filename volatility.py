...
def price_call(S, K, T, sigma, r, q):
    ...
    # C(σ)
    C = S * math.exp(-q * T) * stats.norm.cdf(d1) - K * math.exp(-r * T) * stats.norm.cdf(d2)
    return C


def price_put(S, K, T, sigma, r, q):
    d1 = (math.log(S / K) + (r - q) * T) / (sigma * math.sqrt(T)) + 0.5 * (sigma * math.sqrt(T))
    d2 = (math.log(S / K) + (r - q) * T) / (sigma * math.sqrt(T)) - 0.5 * (sigma * math.sqrt(T))
    P = K * math.exp(-r * T) * stats.norm.cdf(-d2) - S * math.exp(-q * T) * stats.norm.cdf(-d1)
    return P
...
def vega_call(S, K, T, sigma, r, q):
    d1 = (math.log(S / K) + (r - q) * T) / (sigma * math.sqrt(T)) + 0.5 * (sigma * math.sqrt(T))
    # b = stats.norm.pdf(d1)
    vega = S * math.exp(-q * T) * math.sqrt(T) / math.sqrt(2 * math.pi) * math.exp(-(d1 ** 2) * 0.5)
    return vega
